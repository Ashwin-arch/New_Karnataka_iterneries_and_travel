  ...
  return (
    <>
      ...
      <button onClick={onToggle}
        className={`absolute top-3 left-14 z-40 rounded-full shadow-lg border p-2 transition-colors ${
          darkMode
      ...
      {/* Sidebar */}
      <div className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-30 max-md:w-[85vw] w-72 ${bg} backdrop-blur-md border-r ${border} shadow-xl transition-transform duration-300 flex flex-col`}
        ...
        <div className="max-md:p-3 p-4 border-b border-slate-100 flex-shrink-0">
          ...
          <div className={`flex items-center gap-2 mb-3 px-3 py-2 rounded-xl ${darkMode ? "bg-slate-800 text-slate-400" : "bg-slate-100 text-slate-400"} text-xs`}>
            <Search size={12} />
            <span className="max-md:hidden">Use the search bar above to find places</span>
            <span className="md:hidden">Use the search bar on the map to find places</span>
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-3 gap-1.5">
            ...
            {visibleCategories.map(([k, v], index) => {
              ...
              return (
                <button key={k} onClick={() => onCategoryFilter(k)}
                  className={`flex flex-col items-center gap-1 p-2 rounded-xl text-[10px] font-medium transition-all active:scale-95 hover:scale-[1.03] hover:shadow-sm ${
                    isActive
                  ...
                  style={{
                    ...
                    animationDelay: `${(index + 1) * 35}ms`,
                    ...(isActive ? { background: v.color } : {})
                  }}>
                  <div className={`p-1.5 rounded-lg shadow-sm transition-transform duration-200 hover:scale-110 ${
        ...
        <div className="flex-1 overflow-y-auto">
          {sidebarLocations.length === 0 ? (
            ...
            sidebarLocations.map((loc, index) => {
              const color = CATEGORY_COLORS[loc.category] || "#64748b"
              const isSelected = selectedLocation?.id === loc.id
              return (
                <button key={loc.id} onClick={() => onPlaceClick(loc)}
                  className={`sidebar-list-item w-full text-left max-md:px-3 max-md:py-2.5 px-4 py-3 flex items-center gap-3 border-b transition-all duration-200 ${
                  ...
                  <span className="max-md:w-6 max-md:h-6 w-7 h-7 rounded-full flex items-center justify-center text-white max-md:text-[9px] text-[10px] font-bold flex-shrink-0 shadow-sm transition-transform group-hover:scale-110" style={{ background: color }}>{loc.id}</span>
                  ...
                  <ChevronRight size={14} className={`flex-shrink-0 transition-transform duration-200 group-hover:translate-x-1 ${darkMode ? "text-slate-600" : "text-slate-300"}`} />
                </button>
              )
            })
          )}
