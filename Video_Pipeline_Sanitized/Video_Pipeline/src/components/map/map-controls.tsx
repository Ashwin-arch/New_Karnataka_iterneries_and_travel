...
export default function MapControls({
  ...
  return (
    <div className="absolute max-md:right-1 right-3 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-1.5 max-md:gap-1 max-md:max-h-[70vh] max-md:overflow-y-auto">
      {/* Zoom */}
      <div className={`map-control-group max-md:w-10 flex flex-col rounded-xl shadow-lg border overflow-hidden ${
      ...
      <button onClick={onCurrentLocation}
        className={`map-control-btn ${btnClass} relative`}
        ...
        <LocateFixed size={18} className={isTracking ? "text-sky-400 animate-pulse" : ""} />
        {isTracking && (
          <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-sky-400 rounded-full animate-pulse" />
        )}
      </button>
      {isTracking && locationAccuracy != null && (
        <div className={`px-2 py-1 rounded-lg shadow-lg text-xs font-medium text-center map-control-btn ${
      ...
      <button onClick={onFitBounds}
        className={`map-control-btn ${btnClass}`}
        style={{ animationDelay: "200ms" }}
        title="Show all places" aria-label="Show all places">
        <Layers size={18} />
      </button>

      {/* Traffic */}
      <button onClick={onToggleTraffic}
        className={`map-control-btn ${btnClass} ${showTraffic ? (darkMode ? "!text-green-400" : "!text-green-500") : ""}`}
      ...
      <button onClick={onToggleTransit}
        className={`map-control-btn ${btnClass} ${showTransit ? (darkMode ? "!text-purple-400" : "!text-purple-500") : ""}`}
      ...
      <button onClick={onToggleDarkMode}
        className={`map-control-btn ${btnClass} ${darkMode ? "!text-amber-400" : ""}`}
      ...
      <button onClick={onToggleFullscreen}
        className={`map-control-btn ${btnClass} ${isFullscreen ? (darkMode ? "!text-sky-400" : "!text-sky-500") : ""}`}
